...
void ntt2x2_fwdntt(bram *ram, enum OPERATION mode, enum MAPPING mapping)
{
    ...
    uint16_t fifo_w[DEPT_W][DEPT_W] = {0};
    ...
    // Initialize twiddle
    uint16_t tw_i[4] = {0}, tw_base_i[4] = {0};
    ...
    for (unsigned l = 0; l < FALCON_LOGN; l += 2)
    {
        for (unsigned i = 0; i < FALCON_N / 4; ++i)
        {
            ...
            read_write_fifo<uint16_t>(mode, data_fifo, data_in, null, fifo_a,
            ...
            PIPO<DEPT_W, uint16_t>(w, fifo_w, w1, w2, w3, w4);
            /* ============================================== */
    ...
    for (unsigned i = 0; i < DEPT_W; i++)
    {
        ...
        read_write_fifo<uint16_t>(mode, data_in, null, null, fifo_a,
                                 fifo_b, fifo_c, fifo_d, count);
        ...
        // Buffer twiddle
        PIPO<DEPT_W, uint16_t>(w, fifo_w, 0, 0, 0, 0);
