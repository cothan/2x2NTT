...
void read_twiddle(T *w1, T *w2, T *w3, T *w4,
                    enum OPERATION mode, const uint16_t tw_i[4])
{
    auto i1 = tw_i[0];
    auto i2 = tw_i[1];
    auto i3 = tw_i[2];
    ...
    case INVERSE_NTT_MODE:
        *w1 = (-zetas_barrett[i1] + FALCON_Q);
        *w2 = (-zetas_barrett[i2] + FALCON_Q);
        *w3 = (-zetas_barrett[i3] + FALCON_Q);
        *w4 = (-zetas_barrett[i4] + FALCON_Q);
        break;
